...
const TopFansWidget = ({
  ...
  if (isLoading) {
    return <Card className="shadow-sm mb-2 sm:mb-0 w-full">
        ...
        <CardContent className="p-3 sm:p-6 pt-0 space-y-2 sm:space-y-4">
          {[1, 2, 3].map(i => <div key={i} className="flex items-center space-x-2 sm:space-x-4">
  ...
  return <Card className="shadow-sm hover:shadow-md transition-shadow mb-2 sm:mb-0 w-full">
      ...
      <CardContent className="p-3 sm:p-6 space-y-2 sm:space-y-4">
        {displayFans.map((fan, index) => <div key={fan.fan_id} className="flex items-center space-x-2 sm:space-x-3">
