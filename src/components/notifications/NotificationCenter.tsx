...
const NotificationCenter = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('notification-changes').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'notifications'
    }, payload => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('notifications').select('*').eq('recipient_id', userData.user.id).order('created_at', {
        ascending: false
      }).limit(10);
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const {
        ...
        is_read: true
      }).eq('recipient_id', userData.user.id).eq('is_read', false);
      if (error) throw error;
  ...
  return <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative text-[var(--navbar-light-primary)] hover:bg-[var(--navbar-dark-secondary)] group text-slate-900 bg-blue-300 hover:bg-blue-200">
          <Bell className="h-5 w-5 group-hover:animate-bounce-subtle" />
          {unreadCount > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 bg-funky-pink text-white animate-pulse-glow">
              {unreadCount}
            </Badge>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80 p-0" align="end">
        <Card className="border-0 bg-white/90 backdrop-blur-md">
          <CardHeader className="py-3 px-4 flex flex-row justify-between items-center bg-gradient-to-r from-funky-purple/20 to-funky-pink/20">
            ...
            {unreadCount > 0 && <Button variant="ghost" size="sm" onClick={markAllAsRead} className="text-xs hover:bg-funky-purple/20">
                Mark all as read
              </Button>}
          </CardHeader>
          <CardContent className="p-0 max-h-[400px] overflow-y-auto">
            {isLoading ? <div className="py-8 text-center text-gray-500">Loading notifications...</div> : notifications.length === 0 ? <div className="py-12 text-center text-gray-500 flex flex-col items-center">
