...
const GiftCard = memo(({
  ...
  return (
    <div 
      className={cn(
        "relative p-2 sm:p-3 h-full rounded-xl overflow-hidden transition-all duration-300 transform group",
    ...
    >
      ...
      <div className="relative z-10">
        <div className={cn(
          "w-full aspect-square mb-2 sm:mb-3 overflow-hidden rounded-lg transition-all duration-500", 
        ...
        <div className="mt-1 sm:mt-2 relative">
          <h3 className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200 truncate font-display">{gift.name}</h3>
          <p className="text-[10px] sm:text-xs text-funky-purple font-medium">₹{gift.price}</p>
          <Button 
            size="sm" 
            onClick={handleGiftClick} 
            className={cn(
              "mt-1 sm:mt-2 w-full text-[10px] sm:text-xs py-1 px-2 sm:py-2 sm:px-4 transition-all duration-300", 
...
const GiftSection = () => {
  ...
  return (
    <section className="mb-8 sm:mb-12 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 sm:mb-6 gap-3 sm:gap-4 mx-2 sm:mx-[18px]">
        <h2 className="text-xl sm:text-2xl font-bold font-display bg-clip-text text-transparent bg-gradient-to-r from-funky-purple to-funky-pink">Gift Selection</h2>
        ...
        <div className="relative w-full md:w-auto">
          <Input 
            ...
            className="w-full md:w-64 rounded-full bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-funky-purple/20 focus:border-funky-purple/50 pl-8 sm:pl-10 pr-3 sm:pr-4 py-1.5 sm:py-2 text-xs sm:text-sm shadow-sm" 
          />
      ...
      <div className="relative" ref={carouselRef}>
        {isLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 px-2 sm:px-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="p-2 sm:p-3">
                <Skeleton className="w-full aspect-square rounded-lg mb-2 sm:mb-3" />
                <Skeleton className="w-3/4 h-3 sm:h-4 mb-1 sm:mb-2" />
                <Skeleton className="w-1/2 h-2 sm:h-3 mb-1 sm:mb-2" />
            ...
            <CarouselContent className="-ml-2 sm:-ml-4">
              {filteredGifts.length > 0 ? (
                filteredGifts.map((gift) => (
                  <CarouselItem key={gift.id} className="pl-2 sm:pl-4 basis-1/2 sm:basis-1/2 lg:basis-1/4 transition-all duration-300">
                    <GiftCard gift={gift} isMobile={isMobile} />
                  </CarouselItem>
                ))
              ) : (
                <CarouselItem className="pl-2 sm:pl-4 basis-full">
                  <div className="p-4 sm:p-8 rounded-xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-funky-purple/10 text-center">
