...
const HeroCarousel = () => {
  ...
  return <div className="relative pb-2 pt-2 sm:pb-4 sm:pt-6 mx-0 overflow-hidden">
      ...
      loop: true,
      align: "center"
    }} className="w-full max-w-3xl lg:max-w-2xl mx-auto relative" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} ref={carouselRef}>
        <CarouselContent>
          {slides.map((slide, index) => <CarouselItem key={slide.id} className={cn("flex justify-center items-center transition-opacity duration-500", index === currentIndex ? "opacity-100" : "opacity-0 absolute inset-0")}>
        ...
        {slides.length > 1 && <>
            <CarouselPrevious onClick={previousSlide} className="absolute left-2 top-1/2 -translate-y-1/2 z-30 bg-white/80 hover:bg-white text-black border-0 h-8 w-8 rounded-md shadow-lg" />
            <CarouselNext onClick={nextSlide} className="absolute right-2 top-1/2 -translate-y-1/2 z-30 bg-white/80 hover:bg-white text-black border-0 h-8 w-8 rounded-md shadow-lg" />
          </>}
