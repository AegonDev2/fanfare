...
const InfluencerCard = memo(({
  ...
  return <div className="p-3 h-full relative group cursor-pointer transition-all duration-300 transform hover:translate-y-[-5px]" onClick={() => onProfileClick(influencer.id)} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
      ...
      <div className="relative z-10">
        <div className={cn("w-full aspect-square mb-3 overflow-hidden rounded-xl transition-all duration-500", isHovering ? "shadow-lg shadow-funky-purple/20" : "")}>
        ...
        <div className="mt-2 relative">
          ...
          <Button size="sm" variant="secondary" onClick={() => onProfileClick(influencer.id)} className="mt-2 w-full text-xs bg-white dark:bg-gray-800 hover:bg-funky-purple hover:text-white transition-all duration-300 border border-funky-purple/20">
...
const InfluencerSection = ({
  ...
  return <section className="mb-12 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 mx-[18px]">
        ...
        <div ref={searchInputRef} className="relative w-full md:w-auto">
          <Input placeholder="Search Influencers" type="text" value={searchQuery} onChange={handleSearchChange} onFocus={handleSearchFocus} className="w-full md:w-64 rounded-full bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-funky-purple/20 focus:border-funky-purple/50 pl-10 pr-4 py-2 shadow-sm" />
          ...
          {showSuggestions && searchSuggestions.length > 0 && <div className="absolute z-40 top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl border border-funky-purple/10 backdrop-blur-md animate-expand">
              <ul className="max-h-60 overflow-auto py-1">
                {searchSuggestions.map(influencer => <li key={influencer.id} className="px-4 py-2 hover:bg-funky-purple/10 cursor-pointer flex items-center gap-3 transition-colors duration-200" onClick={() => handleSuggestionClick(influencer.id)}>
      ...
      <div className="relative" ref={carouselRef} onMouseEnter={() => handleCarouselHover(true)} onMouseLeave={() => handleCarouselHover(false)}>
        ...
        {filteredInfluencers.length > 0 && <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 -mb-6 flex space-x-2">
            {Array.from({
          length: Math.min(5, Math.ceil(filteredInfluencers.length / visibleCards))
        }).map((_, i) => <button key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${Math.floor(activeSlide / visibleCards) === i ? "w-6 bg-funky-purple" : "bg-funky-purple/30 hover:bg-funky-purple/50"}`} onClick={() => setActiveSlide(i * visibleCards)} aria-label={`Go to group ${i + 1}`} />)}
          </div>}
      </div>
    </section>;
};
