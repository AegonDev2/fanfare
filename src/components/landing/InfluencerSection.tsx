  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div className={cn(
          "w-full aspect-square mb-2 sm:mb-3 overflow-hidden rounded-xl transition-all duration-500",
        ...
        <div className="mt-1 sm:mt-2 relative">
          ...
          <Button 
            ...
            onClick={() => onProfileClick(influencer.id)} 
            className={cn(
              "mt-1 sm:mt-2 w-full text-[10px] sm:text-xs bg-white dark:bg-gray-800 hover:bg-funky-purple hover:text-white transition-all duration-300 border border-funky-purple/20",
              isMobile ? "py-1 px-2" : ""
            )}
          >
  ...
  return (
    <section className="mb-8 sm:mb-12 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 sm:mb-6 gap-3 sm:gap-4 mx-2 sm:mx-[18px]">
        ...
        <div ref={searchInputRef} className="relative w-full md:w-auto">
          <Input 
            ...
            className="w-full md:w-64 rounded-full bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-funky-purple/20 focus:border-funky-purple/50 pl-10 pr-4 py-1.5 sm:py-2 text-sm shadow-sm" 
          />
            ...
            <div className="absolute z-40 top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl border border-funky-purple/10 backdrop-blur-md animate-expand">
              <ul className="max-h-60 overflow-auto py-1">
                {searchSuggestions.map(influencer => (
                  <li 
                    key={influencer.id} 
                    className="px-3 py-1.5 sm:px-4 sm:py-2 hover:bg-funky-purple/10 cursor-pointer flex items-center gap-2 sm:gap-3 transition-colors duration-200" 
      ...
      >
            ...
            {filteredInfluencers.length > 0 ? (
              filteredInfluencers.map((influencer, index) => (
                <CarouselItem 
                  ...
                  className={cn(
                    "pl-2 sm:pl-4",
                    isMobile ? "basis-1/2" : "basis-full sm:basis-1/2 lg:basis-1/4"
                  )}
                >
                  <InfluencerCard 
                    influencer={influencer} 
        ...
        {filteredInfluencers.length > 0 && (
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 -mb-4 sm:-mb-6 flex space-x-1 sm:space-x-2">
              ...
              <button 
                ...
                className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ${
                  Math.floor(activeSlide / visibleCards) === i 
                    ? "w-4 sm:w-6 bg-funky-purple" 
                    : "w-1.5 sm:w-2 bg-funky-purple/30 hover:bg-funky-purple/50"
                }`} 
