...
const InfluencerCard = memo(({
  ...
  return (
    <div 
      className={cn(
        "p-2 h-full relative group cursor-pointer transition-all duration-300 transform hover:translate-y-[-5px]",
    ...
    >
      ...
      <div className="absolute right-2 top-2 z-20 transform transition-all duration-300 scale-0 group-hover:scale-100 origin-top-right" onClick={handleGiftClick}>
        <Button size="icon" className="rounded-full h-5 w-5 sm:h-8 sm:w-8 bg-white dark:bg-gray-900 backdrop-blur-md shadow-lg border border-funky-purple/20 text-funky-pink hover:bg-funky-purple/10">
          <Gift className="h-2.5 w-2.5 sm:h-4 sm:w-4" />
        </Button>
      </div>
      
      <div className="relative z-10">
        <div className={cn(
          "w-full aspect-square mb-1 overflow-hidden rounded-lg transition-all duration-500",
        ...
        <div className="mt-1 relative">
          ...
          <div className="flex items-center text-[10px] text-gray-600 dark:text-gray-300 mt-0.5 space-x-1">
            <span className="font-medium text-funky-purple text-[9px] truncate">{influencer.platform}</span>
            <span>•</span>
            <div className="flex items-center">
              <Users className="h-2 w-2 mr-0.5 text-funky-pink" />
              <span className="text-[9px]">{formatFollowers(influencer.followers)}</span>
            </div>
          </div>
          <Button 
            size="sm" 
            ...
            onClick={() => onProfileClick(influencer.id)} 
            className={cn(
              "mt-1 w-full text-[10px] py-0.5 px-2 h-6 bg-white dark:bg-gray-800 hover:bg-funky-purple hover:text-white transition-all duration-300 border border-funky-purple/20",
            )}
          >
...
const InfluencerSection = ({
  ...
  return (
    <section className="mb-6 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 sm:mb-6 gap-2 px-1">
        <h2 className="text-lg sm:text-2xl font-bold font-display bg-clip-text text-transparent bg-gradient-to-r from-funky-purple to-funky-pink">Discover Influencers</h2>
        ...
        <div ref={searchInputRef} className="relative w-full md:w-auto">
          <Input 
            ...
            className="w-full md:w-64 rounded-full bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-funky-purple/20 focus:border-funky-purple/50 pl-7 sm:pl-10 pr-3 py-1 text-xs h-8 sm:h-10" 
          />
          ...
          {showSuggestions && searchSuggestions.length > 0 && (
            <div className="absolute z-40 top-full left-0 right-0 mt-1 sm:mt-2 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl border border-funky-purple/10 backdrop-blur-md animate-expand">
              <ul className="max-h-48 sm:max-h-60 overflow-auto py-1">
                {searchSuggestions.map(influencer => (
                  <li 
                    key={influencer.id} 
                    className="px-2 py-1 sm:px-4 sm:py-2 hover:bg-funky-purple/10 cursor-pointer flex items-center gap-2 transition-colors duration-200" 
                    onClick={() => handleSuggestionClick(influencer.id)}
                  >
                    <div className="w-5 h-5 sm:w-8 sm:h-8 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 border border-funky-purple/20">
                      {influencer.profile_image ? (
                        <img src={influencer.profile_image} alt={influencer.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center bg-gray-200">
                          <User className="h-2.5 w-2.5 sm:h-4 sm:w-4 text-gray-500" />
                        </div>
                      )}
                    </div>
      ...
      >
          ...
          <CarouselContent className="-ml-1 sm:-ml-4">
            {filteredInfluencers.length > 0 ? (
              filteredInfluencers.map((influencer, index) => (
                <CarouselItem 
                  key={index} 
                  className="pl-1 sm:pl-4 basis-1/2 sm:basis-1/3 lg:basis-1/4"
                >
                  <InfluencerCard 
                    influencer={influencer} 
              ...
              <CarouselItem className="pl-1 sm:pl-4 w-full">
        ...
        {filteredInfluencers.length > 0 && (
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 -mb-2 sm:-mb-6 flex space-x-1">
