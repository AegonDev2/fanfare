...
export default function AdminDashboard() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Transform gift_requests data to match OrderData interface
      const transformedData: OrderData[] = (data || []).map(item => ({
        id: item.id,
  ...
  const fetchStats = async () => {
    try {
      ...
      const totalRevenue = ordersData ? ordersData.reduce((sum, order) => sum + ((order.product_price || 0) + 5.00), 0) : 0;
